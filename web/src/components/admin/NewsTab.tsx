  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div className="flex items-center gap-3"><Newspaper className="text-purple-400" size={24} /><h2 className="text-xl md:text-2xl font-bold">News & Posts</h2></div>
        <button onClick={openAdd} className="flex items-center gap-2 bg-green-600 hover:bg-green-700 px-4 py-2 rounded-xl font-medium text-sm transition"><Plus size={18} /> New Post</button>
      </div>

      {posts.length === 0 ? (
        <div className="bg-gray-900 rounded-2xl p-10 border border-gray-800 text-center text-gray-500"><Newspaper size={48} className="mx-auto mb-4 opacity-30" /><p>No posts yet</p></div>
      ) : (
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 rounded-2xl p-6 w-full max-w-lg border border-gray-700 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-5"><h3 className="text-xl font-bold">{editing ? 'Edit Post' : 'New Post'}</h3><button onClick={() => setModal(false)}><X size={20} /></button></div>
            <div className="space-y-3">
              {[['Title *', 'title'], ['Author', 'user'], ['Image URL', 'image']].map(([label, key]) => (
                <div key={key}><label className="text-sm text-gray-400 mb-1 block">{label}</label>
                  <input value={form[key]} onChange={e => setForm((p: any) => ({ ...p, [key]: e.target.value }))} className="w-full px-3 py-2 bg-gray-800 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-600" /></div>
              ))}
              <div><label className="text-sm text-gray-400 mb-1 block">Content *</label>
                <textarea value={form.caption} onChange={e => setForm((p: any) => ({ ...p, caption: e.target.value }))} rows={4} className="w-full px-3 py-2 bg-gray-800 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-600 resize-none" /></div>
            </div>
            <div className="flex gap-3 mt-5">
              <button onClick={() => setModal(false)} className="flex-1 py-2 border border-gray-700 rounded-xl text-sm">Cancel</button>
              <button onClick={handleSave} className="flex-1 py-2 bg-green-600 hover:bg-green-700 rounded-xl text-sm font-medium flex items-center justify-center gap-2 transition"><Save size={16} /> Publish</button>
            </div>
          </div>
        </div>
      )}
