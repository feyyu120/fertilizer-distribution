    ...
    <div>
      ...
      {messages.length === 0 ? (
        ...
        <div className="space-y-3">
          {messages.map(m => (
            <div key={m._id} className={`bg-gray-900 rounded-2xl p-4 md:p-5 border transition-all ${m.status === 'unread' ? 'border-indigo-800' : 'border-gray-800'}`}>
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <div className="flex items-center gap-2">
                      {m.status === 'unread' ? <Mail size={16} className="text-red-400" /> : <MailOpen size={16} className="text-gray-500" />}
                      <p className="font-semibold">{m.name}</p>
                    </div>
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusBadge[m.status]}`}>{m.status}</span>
                  </div>
                  <p className="text-sm text-gray-400 mb-1">{m.phone}</p>
                  <p className="text-sm text-gray-200 bg-gray-800/60 rounded-xl p-3">{m.message}</p>

                  {m.reply && (
                    <div className="mt-3 bg-green-900/20 border border-green-800/40 rounded-xl p-3">
                      <p className="text-xs text-green-400 font-medium mb-1">Reply from {m.repliedBy}:</p>
                      <p className="text-sm text-gray-300">{m.reply}</p>
                    </div>
                  )}
                  ...
                  {replyingId === m._id && (
                    <div className="mt-3 flex gap-2">
                      <textarea
                        ...
                        rows={2}
                        className="flex-1 px-3 py-2 bg-gray-800 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-600 resize-none"
                      />
                      <div className="flex flex-col gap-2">
                        <button onClick={() => handleReply(m._id)} className="p-2 bg-green-600 hover:bg-green-700 rounded-xl transition"><Send size={16} /></button>
                        <button onClick={() => { setReplyingId(null); setReplyText(''); }} className="p-2 bg-gray-700 hover:bg-gray-600 rounded-xl transition"><X size={16} /></button>
                      </div>
                    </div>
                  )}
                ...
                <div className="flex gap-2 md:flex-col">
                  {m.status !== 'replied' && (
                    <button
                      ...
                      className="flex items-center gap-1 px-3 py-2 bg-indigo-600/80 hover:bg-indigo-700 rounded-xl text-sm transition"
                    >
                      <Reply size={15} /> Reply
                    </button>
                  )}
                  <button onClick={() => handleDelete(m._id)} className="flex items-center gap-1 px-3 py-2 bg-red-600/70 hover:bg-red-700 rounded-xl text-sm transition">
